...
FIXED: Proper scene clearing to prevent data block collisions
"""
...
def retarget_animation(
    ...
    armature_obj.data.animation_data_create()
    armature_obj.data.animation_data.action = action
...
def process_npz_file(npz_path: Path) -> None:
    ...
    # Create T-pose armature with UNIQUE name per file - FIXED
    unique_name = f"SMPLH_Armature_{npz_path.stem}"
    armature = create_tpose_armature(unique_name)
    
    # Set frame range
    bpy.context.scene.frame_start = 0
    bpy.context.scene.frame_end = len(poses) - 1
    
    # Retarget animation with unique action name - FIXED
    action_name = f"Action_{unique_name}"
    retarget_animation(armature, poses, trans, action_name)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Retarget SMPL+H animations to consistent T-pose skeleton (FIXED: proper scene clearing)")
